...
class SimpleMySQLi {
	 ...
	 */
//	 private $settings = require '../src/settings.php';
//$db_user = $settings["settings"]["db"]['user'];
//$db_pass = $settings["settings"]["db"]['pass'];
//$db_host = $settings["settings"]["db"]['host'];
//$db_name = $settings["settings"]["db"]['name'];

	public function __construct($host = '', $username = '', $password = '', $dbName = '', $charset = 'utf8mb4', $defaultFetchType = 'assoc') {
		
		global $settings;
		if($host == '' && $username == '' && $password == '' && $dbName == ''){
			$host = $settings["settings"]["db"]['host'];
			$dbName = $settings["settings"]["db"]['name'];
	 ...
	 * @throws mysqli_sql_exception If any mysqli function failed due to mysqli_report(MYSQLI_REPORT_ERROR | MYSQLI_REPORT_STRICT)
	 */
	public function query($sql, $values = [], $types = '') {
	 ...
	 * @throws mysqli_sql_exception If any mysqli function failed due to mysqli_report(MYSQLI_REPORT_ERROR | MYSQLI_REPORT_STRICT)
	 */
	public function execute($values = [], $types = '') {
		if(!is_array($values)) $values = [$values]; //Convert scalar to array
	 ...
	 * @throws mysqli_sql_exception If mysqli function failed due to mysqli_report(MYSQLI_REPORT_ERROR | MYSQLI_REPORT_STRICT)
	 */
	public function rowsMatched() {
		return $this->isset(info()['Rows matched']) ? info()['Rows matched'] : false;
	}
	...
	* @throws mysqli_sql_exception If any mysqli function failed due to mysqli_report(MYSQLI_REPORT_ERROR | MYSQLI_REPORT_STRICT)
	*/
	public function fetch($fetchType = '', $className = 'stdClass', array $classParams = []) {
	...
	* @throws mysqli_sql_exception If any mysqli function failed due to mysqli_report(MYSQLI_REPORT_ERROR | MYSQLI_REPORT_STRICT)
	*/
	public function fetchAll($fetchType = '', $className = 'stdClass', array $classParams = []) {
		$stmtResult = $this->stmtResult;
		$arr = [];
	 ...
	 */
	public function atomicQuery($sql, array $values, array $types = []) {
		try {
